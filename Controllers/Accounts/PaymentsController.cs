...
namespace IcampusBoatBackend.Controllers.Accounts
{
    ...
    public class PaymentsController : ControllerBase
    {
        ...
        /// <summary>
        /// Generates next voucher number for financial year
        /// Parameters: 1 (<= 2, uses method param)
        /// </summary>
        [AllowAnonymous]
        [HttpGet]
        [Route("GetVoucherNo")]
        public IActionResult GetVoucherNo(string? fYear)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(DAL.SQLConnString))
                {
                    con.Open();
                    string query = "SELECT ISNULL(MAX(DBO.VAL(VOUCHERNO)), 0) + 1 AS VOUCHERNO FROM TBL_PAYMENTS WHERE FINANCIALYEAR = @FYEAR";
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        cmd.CommandType = CommandType.Text;
                        cmd.Parameters.AddWithValue("@FYEAR", (object?)fYear ?? DBNull.Value);
                        var voucherNoObj = cmd.ExecuteScalar();
                        string voucherNo = (voucherNoObj != null && voucherNoObj != DBNull.Value) ? voucherNoObj.ToString()! : "1";
                        return Ok(new { voucherNo = voucherNo });
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Retrieves payments grid data for academic and financial year
        /// Parameters: 2 (<= 2, uses method params)
        /// </summary>
        [AllowAnonymous]
        [HttpGet]
        [Route("GetPaymentData")]
        public IActionResult GetPaymentData(string? academicYear, string? fYear)
        {
            try
            {
                var result = new List<object>();
                ...
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_GETPAYMENT_DATA", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@ACADEMICYEAR", (object?)academicYear ?? DBNull.Value);
                        cmd.Parameters.AddWithValue("@FYEAR", (object?)fYear ?? DBNull.Value);
        ...
        /// <summary>
        /// Retrieves voucher details by voucher number and financial year for view/reprint
        /// Parameters: 2 (<= 2, uses method params)
        /// </summary>
        [AllowAnonymous]
        [HttpGet]
        [Route("GetVoucherData")]
        public IActionResult GetVoucherData(string? voucherNo, string? fYear)
        {
            try
            {
                var result = new List<object>();
                ...
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_GETVOUCHER_DATA", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
        ...
        /// <summary>
        /// Saves payment transaction details
        /// Parameters: 16 (> 2, uses [FromBody] Payments model)
        /// </summary>
        [AllowAnonymous]
        [HttpPost]
        [Route("SavePayment")]
        public IActionResult SavePayment([FromBody] Payments model)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(DAL.SQLConnString))
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("SP_PAYMENT_SAVE", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        cmd.Parameters.AddWithValue("@PID", (object?)model.PId ?? DBNull.Value);
