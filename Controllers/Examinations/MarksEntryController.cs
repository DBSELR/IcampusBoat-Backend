...
namespace IcampusBoatBackend.Controllers.Examinations
{
    ...
    public class MarksEntryController : ControllerBase
    {
                ...
                using (SqlConnection con = new SqlConnection(DAL.SQLConnString))
                {
                    ...
                    return Ok(new
                    {
                        ...
                        data = new
                        {
                            ...
                            midExams = JsonConvert.SerializeObject(midExams)
                        }
                    });
                }
            }
        ...
        [HttpGet]
        [Route("subjects")]
        public IActionResult GetSubjects([FromQuery] SubjectFilterModel request)
        {
            try
            {
                using SqlConnection con = new SqlConnection(DAL.SQLConnString);
        ...
        [HttpGet]
        [Route("mid-types")]
        public IActionResult GetMidTypes([FromQuery] MidTypeFilterModel request)
        {
            try
            {
                using SqlConnection con = new SqlConnection(DAL.SQLConnString);
        ...
        [HttpGet]
        [Route("max-min-marks")]
        public IActionResult GetMaxMinMarks([FromQuery] MaxMinMarksFilterModel request)
        {
            try
            {
        ...
        public IActionResult CheckInternalDates([FromQuery] InternalDateFilterModel request)
        {
            try
            {
                using SqlConnection con = new SqlConnection(DAL.SQLConnString);
        ...
        [HttpGet]
        [Route("student-marks")]
        public IActionResult GetStudentMarks([FromQuery] StudentMarksFilterModel request)
        {
            try
            {
        ...
        public IActionResult ValidateRegNo([FromQuery] ValidateRegNoFilterModel request)
        {
            try
            {
                using SqlConnection con = new SqlConnection(DAL.SQLConnString);
        ...
        [HttpPost]
        [Route("save-All")]
        public IActionResult SaveAll([FromBody] MarksEntrySaveModel request)
        {
            try
            {
                if (request == null)
        ...
        [HttpPost]
        [Route("save")]
        public IActionResult Save([FromBody] MarksEntryAttendanceSaveModel request)
        {
            try
            {
        ...
        private static DataTable ExecuteSpWithFallback(SqlConnection con, List<(string spName, Dictionary<string, object?> paramsDict)> spCandidates)
        {
            foreach (var (spName, paramsDict) in spCandidates)
            {
                ...
                {
                    // 2812/15009 = Procedure not found
                    // 8144/201   = Procedure has too many arguments specified or wrong params
                    continue;
                }
            }
